...
export const createEventFlowGuide = (eventId, options = {}) => {
  ...
  const canViewEventDashboard = Boolean(permissions.canViewEventDashboard);
  const canManageEvent = Boolean(permissions.canManageEvent);
  const canCreateTasks = Boolean(permissions.canCreateTasks);
  const canViewDepartments = Boolean(permissions.canViewDepartments);
  const canManageDepartments = Boolean(permissions.canManageDepartments);
  ...
  const steps = [
    ...
    canViewDepartments && {
      ...
      title: 'Ban tổ chức',
      content: 'Mở trang quản lý nhóm phụ trách có thể truy cập trong sự kiện.',
